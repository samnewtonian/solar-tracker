...
> All three implementations accept timezone-aware datetime objects (`ZonedDateTime`, `datetime`, `chrono::DateTime<Tz>`) and convert to UTC internally.
...
**Rust:** Rust 1.80+ (depends on `chrono`)
...
let dt = cst.with_ymd_and_hms(2026, 3, 21, 12, 0, 0).unwrap();
...
// Single-axis tracker rotation
let dt2 = cst.with_ymd_and_hms(2026, 3, 21, 15, 0, 0).unwrap();
