...
**API update**: These now all accept timezone-aware datetime objects (Clojure/Java `ZonedDateTime`, Python `datetime`, Rust `chrono::DateTime<Tz>`) in any call that wants a date and time. The API consumer examples include using IANA standard time zone names (e.g. America/Chicago) rather than fixed offsets from UTC.

**Important**: This codebase is almost entirely generated by Claude Code. While I've made many efforts to make sure it keeps reviewing its own code (`correctness, factoring, and optimization` has been typed a lot recently) I have not done a Mk I Eyeball review or tried using the API to build an application yet. While the API is meant to be pure (as in pure functions), use at your own risk! Or help clean it up :)
...
**Clojure:** Clojure 1.12+, Java 11+ (`java.time` and `spec.alpha`)

**Python:** Python 3.11+ (no external dependencies; `pytest` for dev)

**Rust:** Rust 1.80+ (depends on `chrono`; `chrono_tz` for dev)
...
chrono-tz = "0.10"  # Recommended for named timezones (e.g. America/Chicago)
```
...
See [`doc/api-reference.md`](doc/api-reference.md) for the full cross-language API reference and [`doc/internal-helpers.md`](doc/internal-helpers.md) for internal implementation details.

Python and Rust naming follows language conventions (snake_case, `&` references in Rust). Clojure uses kebab-case (e.g. `solar-position`).

### Core Solar Position (`angles`)

| Function | Description |
|----------|-------------|
| `solar-position(lat, lon, datetime)` | Full solar position; converts to UTC internally |
